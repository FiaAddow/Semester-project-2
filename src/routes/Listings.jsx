  ...
  const handleSearch = (e) => {
    ...
    const fetchSearched = async () => {
      const response = await fetch(
        `${API}/auction/listings/search?q=${e.target[0].value}&limit=6`,
      );
      ...
      <div className="@container mx-auto p-5 sm:p-10 md:p-16">
        <div className="mb-6 w-1/2 mx-auto">
          <form onSubmit={handleSearch} className="w-full inline-flex">
            ...
            />
            <button
              className="p-2 mb-4 font-fairy2 rounded-lg ml-4 text-fairy-1 bg-fairy-4 px-3 py-1.5 text-sm/6 font-semibold hover:bg-fairy-3 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-fairy-3"
              type="submit"
            >
              Search
            </button>
                    ...
                    <div className="-rounded overflow-hidden shadow-lg flex flex-col ">
                      ...
                      >
                        ...
                        <div className=" px-6 py-4 mb-auto shadow-lg rounded-lg bg-f mt-4 bg-fairy-1 min-h-64">
                          <h2 className="font-medium text-lg  hover:text-fairy-3 transition duration-500 ease-in-out inline-block mb-2 capitalize">
                            {listing.title}
                          </h2>
                          <p className="text-fairy-5 mb-2 ">
                            {listing.description}
                          </p>
                        </div>
                      </div>
