...
export default function Listing() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(`${API}/auction/listings/${id}?_bids=true`);
  ...
  if (listing) {
    return (
      <div className="@container p-10 mt-42 mx-6 md:mx-12 lg:mx-24 border border-fairy-4 bg-white">
        ...
        <div className="flex flex-col items-center gap-8" key={listing.id}>
          ...
          <div className="flex flex-col items-center gap-4">
            <h1
              className="text-2xl font-fairy-2 text-fairy-5 text-center capitalize"
              key={listing.id}
            >
              {listing.title}
            </h1>
          ...
          <div className="">
            <h1 className="font-fairy text-2xl text-fairy-4 text-center">
              Bids:
            </h1>
            <div>
