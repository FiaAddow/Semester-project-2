...
export default function Login() {
  ...
  const handleSubmit = async (event) => {
    ...
    console.log("Form submitted", event.target.email.value, event.target.password.value);
    ...
    <div className="h-100vh">
      <div className="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
        <div className="sm:mx-auto sm:w-full sm:max-w-sm">
          <h2 className="mt-10 text-center text-2xl/9 font-bold tracking-tight text-fairy-5">Sign in to your account</h2>
        </div>
        ...
        <div className="mt-10 bg-white p-8 rounded-2xl sm:mx-auto sm:w-full sm:max-w-sm">
          <form onSubmit={handleSubmit} method="POST" className="space-y-6">
            ...
            <div>
              <button
                type="submit"
                className="flex w-full justify-center rounded-md bg-fairy-4 px-3 py-1.5 text-sm/6 font-semibold text-white hover:bg-fairy-3 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-fairy-3"
              >
                Sign in
              </button>
            </div>
